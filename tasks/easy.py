...
class EasyTask(BaseTask):
    config = TaskConfig(
        ...
        max_steps=30,
        initial_threat_count=1,
        resource_per_step=1.5,          # 50% more resources than default
        attack_progression_prob=0.12,   # slow kill-chain advancement
        lateral_spread_base_prob=0.08,  # low spread risk
        natural_severity_growth=0.02,
        health_degradation_rate=0.03,
        false_positive_rate=0.05,       # minimal noise
        false_negative_rate=0.08,
        base_detection_prob=0.75,       # high detection confidence
        passing_score=0.60,
    )
