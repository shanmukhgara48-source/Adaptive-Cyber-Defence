...
against three entry points simultaneously.  The attacker uses evasion techniques
...
class HardTask(BaseTask):
    config = TaskConfig(
        ...
        difficulty="hard",
        description=(
            "Coordinated APT across 3 entry points. Low detection, high false-positive "
        ...
        attack_progression_prob=0.22,   # fast kill-chain (reduced from 0.38 — old value caused PHISHING→EXFIL in 5 steps)
        ...
        false_negative_rate=0.30,       # attacker evades detection often
        base_detection_prob=0.38,       # low baseline detection
        passing_score=0.45,
    )
